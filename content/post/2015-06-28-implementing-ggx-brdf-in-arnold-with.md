...
For getting better understanding of the task of these functions. Recall that our goal is to use MIS to combine the samples generated from light's PDF and BRDF's PDF. Let BRDF as $f(X)$ and lighting function as $g(Y)$ in equation \eqref{mis}, then the unknown entities to Arnold are:
